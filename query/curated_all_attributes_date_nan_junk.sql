...
AND (curated_product_fields."name") IS NULL OR (curated_product_fields."name" ) ='None'
...
--     products."external_id"  AS "products.external_id",
...
-- LEFT JOIN public.attributes  AS attributes ON (curated_product_fields."name") = (attributes."snake_case_name")
-- LEFT JOIN public.buckets  AS buckets ON (strategy_buckets."bucket_id") = (buckets."id")
-- WHERE (curated_product_fields."customer_id" ) = 5 AND (products."active" ) AND (attributes."snake_case_name" ) IS NULL
-- GROUP BY
--     1,
--     2,
--     3,
